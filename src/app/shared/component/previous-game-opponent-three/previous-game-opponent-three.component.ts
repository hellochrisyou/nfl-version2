...
export class PreviousGameOpponentThreeComponent {
  ...
  sortColumn(event: any) {
    switch (event.active) {
      ...
      case 'result': {
        if (event.direction === "asc") {
            this.tmpGames.sort((a: Game, b: Game) => ((a.threePtValue! - this.returnPlayerAvg(a.opponentPlayer!)) < (b.threePtValue! - this.returnPlayerAvg(b.opponentPlayer!)) ? -1 : 1));
            this.dataSource = new MatTableDataSource(this.tmpGames);
          }
         else if (event.direction === "desc") {
          this.tmpGames.sort((a: Game, b: Game) => ((a.threePtValue! - this.returnPlayerAvg(a.opponentPlayer!)) > (b.threePtValue! - this.returnPlayerAvg(b.opponentPlayer!)) ? -1 : 1));
      ...
      case 'value': {
        if (event.direction === "asc") {
            this.tmpGames.sort((a: Game, b: Game) => ((a.threePtValue!) < (b.threePtValue!) ? -1 : 1));
            this.dataSource = new MatTableDataSource(this.tmpGames);
        } else if (event.direction === "desc") {
            this.tmpGames.sort((a: Game, b: Game) => ((a.threePtValue!) > (b.threePtValue!) ? -1 : 1));
            this.dataSource = new MatTableDataSource(this.tmpGames);
